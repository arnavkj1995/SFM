...
class TimeStepToGymWrapper:
    def __init__(self, env, max_episode_steps, obs_type='states', domain='quadruped', task='walk'):
        self.env = env
        ...
        self.action_space = gym.spaces.Box(
            low=np.full(self.env.action_spec().shape, env.action_spec().minimum),
            high=np.full(self.env.action_spec().shape, env.action_spec().maximum),
            dtype=env.action_spec().dtype)

    def reset(self):
        self.t = 0
        ts = self.env.reset()
        return ts.observation, defaultdict(float)

    def step(self, action):
        self.t += 1
        ts = self.env.step(action)
        return ts.observation, ts.reward, ts.last(), self.t == self.max_episode_steps, defaultdict(float)
